import openpyxl,re

# openpyxl, 索引从1开始。这些数据所有单映射表通用
StartRow = 4
OriginCol = 1
NewCol = 3

multiMapTable = {}
try:
    wb = openpyxl.load_workbook('../data/map/multiMap.xlsx')
    ws = wb.get_sheet_by_name(wb.sheetnames[0])
    for row in range(StartRow,ws.max_row+1):
        origin = ws.cell(row,OriginCol).value
        if not origin:
            continue
        for col in range(NewCol,ws.max_column+1):
            new = ws.cell(row,col).value
            if new:
                multiMapTable.setdefault(origin,[]).append(new)
    wb.close()
except:
    print("多课程映射表读取失败，使用内置数据")
    multiMapTable = {
        "41000110A":[
            "Read1",
            "Listen1"
        ],
        "41000110B":[
            "Read2",
            "Listen2"
        ]
    }

try:
    wb = openpyxl.load_workbook('../data/map/singleMap.xlsx')
    ws = wb.get_sheet_by_name(wb.sheetnames[0])
    singleMapTable = {}
    for row in range(StartRow,ws.max_row+1):
        origin = ws.cell(row,OriginCol).value
        if not origin:
            continue
        new = ws.cell(row,NewCol).value
        singleMapTable[origin]=new
    wb.close()
except:
    print("单映射表读取失败，使用内置数据")
    singleMapTable = {
        # 第一学期，倒数第二位2->读写，3->听说，最后一位是层次
        "00020021A":"Read1",
        "00020022A":"Read1",
        "00020023A":"Read1",
        "00020031A":"Listen1",
        "00020032A":"Listen1",
        "00020033A":"Listen1",
        # 第二学期一、三层次常规课
        "00020021B":"Read2",
        "00020031B":"Listen2",
        "00020023B":"Read2",
        "00020033B":"Listen2",
        # 听说二层
        "00020032B1":"Listen2",  # 高级英语口语
        "00020032B2":"Listen2",  # 视听说与跨文化交际
        "00020032B3":"Listen2",  # 国际学术交流英语
        "00020032B4":"Listen2",  # 英文电影视听说与文化赏析
        # 读写二层
        "00020022B1":"Read2",  # 新闻英语读写
        "00020022B2":"Read2",  # 翻译基础与实践
        "00020022B3":"Read2",  # 批判性阅读：美国社会与文化
        "00020022B5":"Read2",  # 学术性阅读：语言与思维
        "00020022B6":"Read2",  # 学术英语写作
    }

PERegex = re.compile(r'00040.*?')
PENumbers = (
    '00040010A',
    '00040010B',
    '00040010C',
    '00040010D',
)

wb = openpyxl.load_workbook('../data/map/substitute.xlsx')
ws = wb.get_sheet_by_name(wb.sheetnames[0])

substituteMapTable = {}

for row in range(StartRow,ws.max_row+1):
    originNumber = ws.cell(row,OriginCol).value
    newNumber = ws.cell(row,NewCol).value
    if originNumber:
        substituteMapTable[originNumber]=newNumber
